...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 6.3: benign installation recovery latency.")
    ...
    for trial in range(total_runs):
        context = build_context(args.var, trial=trial + 1, scenario="install")
        ...
        if trial >= args.warmups:
            results.append(
                TrialResult(
                    experiment="exp_06_benign_install_recovery",
                    trial=trial - args.warmups + 1,
                    success=success,
                    metric_name="pending_to_ready_latency",
    ...
    finalize_results("exp_06_benign_install_recovery", results, args.output_dir)
