...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 6.2: stale rollback denial latency.")
    ...
    for trial in range(total_runs):
        context = build_context(args.var, trial=trial + 1, scenario="stale_rollback")
        ...
        if trial >= args.warmups:
            results.append(
                TrialResult(
                    experiment="exp_02_stale_rollback",
                    trial=trial - args.warmups + 1,
                    success=success,
                    metric_name="rollback_to_denial_latency",
    ...
    finalize_results("exp_02_stale_rollback", results, args.output_dir)
