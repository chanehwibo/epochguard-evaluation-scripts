...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 6.3: plugin activation P95 latency.")
    ...
    context = build_context(args.var, workload="plugin_activation")
    materialize_input_file(
    ...
    results = [
        TrialResult("exp_16_plugin_activation_latency", index + 1, True, "latency", value, "ms", "plugin_activation")
        for index, value in enumerate(latencies)
    ]
    finalize_results("exp_16_plugin_activation_latency", results, args.output_dir)
